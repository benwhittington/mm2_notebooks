...
def runge_step(f, xk, yk, h, *args):
    """
		completes a single step of the runge kutta method (K4)
...
def runge_solve(f, x0, y0, x1, h, *args):
    '''
		solves a differential equation using runge kutta
...
def RunMethod(h, steps, eqnKey, method, showSln, showGrad):
    ...
    if method == 'Improved Euler':  # call to improved euler
        ...
        if showGrad:
            plotTangentAtPoint(ax, f, x[-1] + h, euler_step(f, x[-1], y[-1], h), x[-1] + h, xMax, h,
                               label='Corrector Step', color='r')
    ...
    elif method == 'Runge Kutta':  # call to runge Kutta
...
def NumericalMethods():
    ...
    solver_drp = widgets.Dropdown(options=['Euler', 'Improved Euler', 'Runge Kutta'], description='Solver Type')
...
def Laplace():
    ...
    # define widgets
    tlim_sldr = widgets.FloatRangeSlider(value=[-10, 10], step=0.1, min=-tAbsLim, max=tAbsLim, description='t lim',
                                         continuous_update=False)
    slim_sldr = widgets.FloatRangeSlider(value=[-10, 10], step=0.1, min=-sAbsLim, max=sAbsLim, description='s lim',
                                         continuous_update=False)
    fslim_sldr = widgets.FloatRangeSlider(value=[0, 5], step=0.1, min=-5, max=5, description='F(s) lim',
                                          continuous_update=False)
    tShift_sldr = widgets.FloatSlider(value=0, step=0.1, min=-2, max=2, description='Shift t',
                                      continuous_updating=False)
    sShift_sldr = widgets.FloatSlider(value=0, step=0.1, min=-2, max=2, description='Shift s',
                                      continuous_updating=False)
    func_drop = widgets.Dropdown(options={'t^n': 'Poly', 'sin(w*t)': 'Sine', 'cos(w*t)': 'Cos', 'exp(at)': 'exp'},
                                 layout=widgets.Layout(width='220px'), description='$f(t)=$')
...
def Pendulum(a0=0, v0=0):
    ...
    # interval = 5
    anim = animation.FuncAnimation(fig, pendUpdate, frames=150, interval=50, fargs=[aVec, vVec, line], repeat=True,
                                   blit=True)
